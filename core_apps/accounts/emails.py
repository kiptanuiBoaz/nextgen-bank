...
def transfet_email(
    ...
    subject = _("Transefer Notification")
    from_email = settings.DEFAULT_FROM_EMAIL
    ...
    sender_html_email = render_to_string(
        "emails/transer_notification.html", sender_context
    )
    ...
    receiver_html_email = render_to_string(
        "emails/transer_notification.html", receiver_context
    )
    ...
    receiver_email_obj.attach_alternative(sender_html_email, "text/html")

    try:
        sender_email_obj.send()
        receiver_email_obj.send()
        logger.info(
            f"Transefer  notification  email sent to: {sender_email}"
            f"{receiver_email}"
        )
    except Exception as e:
        logger.error(f"Failed to send transfer notification emails to. Error: {str(e)}")


def send_transfer_otp_email(email, otp) -> None:
    subject = _("Your OTP code for Transfer Authorization")
    from_email = settings.DEFAULT_FROM_EMAIL
    ...
    html_email = render_to_string("emails/transfer_otp.html", context)
    plain_email = strip_tags(html_email)
    email = EmailMultiAlternatives(subject, plain_email, from_email, recipient_list)
    email.attach_alternative(html_email, "text/html")
    try:
